    ...
    exit();
      ...
      $sql = "SELECT * FROM tradusers";
			...
			for($i=0; $i<$resultcheck; $i++){
				...
				//to be used
					$stmt = $conn->prepare("INSERT INTO tradusers (username, password, email, active) VALUES (?, ?, ?, ?)");
					$stmt->bind_param("sssi", $username,$passwordtodb, $email, $active);
					...
					echo '<h3 style="text-align: center; margin-left: 40px;">The user email <?php echo $useremail; ?> has been successfully registered!</h3>';
					die();
